...
export class PaypalAdapter {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    if (!response.ok) {
      ...
      console.error(
        '❌ [PAYPAL AUTH ERROR]: Credenciales incorrectas en el .env',
        errData,
      );
  ...
  async createOrder(
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          '❌ [PAYPAL API ERROR]: PayPal rechazó el cuerpo del JSON:',
          JSON.stringify(data, null, 2),
        );
        throw new HttpException(
          'Error al crear orden en servidores de PayPal',
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      }
      ...
      throw new HttpException(
        'Fallo crítico en el adaptador de PayPal',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async capturePayment(paypalOrderId: string) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new HttpException(
          'Error al capturar el pago en los servidores de PayPal',
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      }
      ...
      throw new HttpException(
        'Fallo crítico al procesar la captura de PayPal',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
