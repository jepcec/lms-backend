...
import type { CourseStatus, CourseLevel, CourseCurrency, CourseAccessDuration } from "../../domain/course.entity";
...
export class UpdateCourseUseCase {
	...
	async execute(id: string, dto: UpdateCourseDto) {
		...
		const published_at = (dto.status === 'published' && !course.published_at) ? new Date() : course.published_at
		...
		const updated = new CourseEntity({
			...
			discount_price: dto.discount_price !== undefined ? dto.discount_price : course.discount_price,
			currency: (dto.currency as CourseCurrency) ?? course.currency,
			access_duration: (dto.access_duration as CourseAccessDuration) ?? course.access_duration,
