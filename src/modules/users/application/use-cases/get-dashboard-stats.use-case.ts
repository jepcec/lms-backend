...
export class GetDashboardStatsUseCase {
	...
	async execute(filters: AdminFiltersDto) {
		...
		const [
			...
			this.prisma.user.count({ where: { role: "estudiante", status: "active" } }),
			this.prisma.user.count({
				where: { role: "estudiante", created_at: { gte: startOfMonth } },
			}),
			this.prisma.course.count({ where: { status: "published", deleted_at: null } }),
			this.prisma.course.count({
				where: { status: "published", created_at: { gte: startOfMonth }, deleted_at: null },
			}),
		...
		const total_mes_anterior = ingresosMesAnterior.online + ingresosMesAnterior.manual;
		const cambio_porcentual = total_mes_anterior > 0
			? ((total_mes - total_mes_anterior) / total_mes_anterior) * 100
			: total_mes > 0 ? 100 : 0;

		const tasa_finalizacion = totalEnrollments > 0
