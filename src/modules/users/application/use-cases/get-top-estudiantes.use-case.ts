		...
		const result = estudiantes
			.map((user) => {
				const totalWatchedSeconds = user.enrollments.reduce(
					(sum, e) => sum + e.lesson_progress.reduce((s, lp) => s + lp.watched_seconds, 0),
					0,
				);
				return {
					id: user.id,
					...
					completed_count: user.enrollments.filter((e) => e.completed_at).length,
					total_watched_hours: Math.round((totalWatchedSeconds / 3600) * 100) / 100,
				};
			})
