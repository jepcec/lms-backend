...
export class SetGradeUseCase {
  ...
  async execute(enrollmentId: string, score: number) {
    ...
    if (!enrollment) {
      throw new NotFoundException(`No se encontró la matrícula con ID: ${enrollmentId}`);
    }
    ...
    if (enrollment.progress_percent.toNumber() < 100) {
      throw new BadRequestException('El estudiante aún no completa el 100% del curso');
    }
    ...
    if (!activeTemplate) {
      throw new BadRequestException('No hay una plantilla de certificado activa en el sistema');
    }

    // 4. Determinar tipo de documento según la nota mínima (13)
    const type = score >= 13 ? CertificateType.Certificado : CertificateType.Constancia;
