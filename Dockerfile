...
CMD ["sh", "-c", "set -e; echo \"[$(date +%H:%M:%S)] STEP 1/4: Verificando DATABASE_URL...\"; if [ -z \"$DATABASE_URL\" ]; then echo \"[$(date +%H:%M:%S)] FATAL: DATABASE_URL no definida\"; exit 1; fi; echo \"[$(date +%H:%M:%S)] OK: DATABASE_URL detectada\"; echo \"[$(date +%H:%M:%S)] DATABASE_URL: $(echo $DATABASE_URL | sed 's/:[^:@]*@/:***@/')\"; echo \"[$(date +%H:%M:%S)] STEP 2/4: Listando archivos de Prisma...\"; ls -la prisma/ 2>&1; echo \"[$(date +%H:%M:%S)] STEP 3/4: Aplicando migraciones...\"; npx prisma migrate deploy; echo \"[$(date +%H:%M:%S)] OK: Migraciones aplicadas\"; echo \"[$(date +%H:%M:%S)] STEP 4/4: Iniciando NestJS...\"; exec node dist/src/main.js"]
